...
class MyDataset(Dataset):
    ...
    read_data_from_file([data_file_path]): [numpy.ndarray of uint8, list of int]
    ...
    def __init__(self, data_file_path, transform):
        ...
        self.features, self.fine_labels = self.read_data_from_file(data_file_path)
    ...
    def __getitem__(self, index):
        ...
        fine_label: int
            fine label of the wanted instance
        """
    ...
    def get_class(self, cnt):
        ids_needed = np.argwhere(np.array(self.fine_labels) < cnt)[:, 0]
        tmp_features = np.array(self.features)[ids_needed, :]
        tmp_labels = np.array(self.fine_labels)[ids_needed]
        self.features = tmp_features
        self.fine_labels = list(tmp_labels)


# debug test
if __name__ == "__main__":
    data_file_path = '../datasets/CIFAR100/train'
